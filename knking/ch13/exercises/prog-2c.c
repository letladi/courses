...
const char months[MONTHS_IN_YR][3] = {
...
int monthcmp(char*, char*);

int main(void)
{
   char reminders[MAX_REMIND][MSG_LEN + TIME_LEN + 3];
   char day_str[MONTH_NAME_LEN + 4], msg_str[MSG_LEN + 1];
   int day, month, i, j, hr, min, time, num_remind = 0;
   ...
   for ( ; ; ) {
      ...
      if (month <= 0 || month > 12) {
         printf("month value must be between 1 and 12, you entered %d. Please try again...\n", month);
         continue;
      }
      ...
      int day_cmp, time_cmp, month_cmp;
      for (i = 0; i < num_remind; i++) {
         
         month_cmp = monthcmp(day_str, reminders[i]);
         if (month_cmp < 0) break;
         else if (month_cmp == 0) {
            while ((day_cmp = daycmp(day_str, reminders[i])) > 0) i++;
            if (day_cmp < 0) break;
            else if (day_cmp == 0) {
...
int monthcmp(char* t1, char* t2)
{
   return get_month(t1) - get_month(t2);
}