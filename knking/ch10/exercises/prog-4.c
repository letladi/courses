...
void analyze_hand(void)
{
   ...
   for (int i = 0; i < NUM_RANKS; i++) {
      switch(i) {
         case ACE: royal = royal && (num_in_rank[i] > 0);
         case KING: royal = royal && (num_in_rank[i] > 0);
         case QUEEN: royal = royal && (num_in_rank[i] > 0);
         case JACK: royal = royal && (num_in_rank[i] > 0);
         case TEN: royal = royal && (num_in_rank[i] > 0);
      }
      if (!royal) break;
   }
