...
int find_part(struct part inventory[], int len, int number);
int insert(struct part inventory[], int len);
void search(struct part inventory[], int len);
void update(struct part inventory[], int len);
void print(struct part inventory[], int len);
 ...
 int main(void)
 {
    ...
    for (;;) {
      ...
      while (getchar() != '\n') /* skips to end of line */
         ;
      switch(code) {
         case 'i': num_parts += insert(inventory, num_parts); break;
...
int insert(struct part inventory[], int len)
{
   ...
   if (find_part(inventory, len, part_number) >= 0) {
      printf("Part already exists.\n");
      return 0;
   }

   inventory[len].number = part_number;
   printf("Enter part name: ");
   read_line(inventory[len].name, NAME_LEN);
   printf("Enter quantity on hand: ");
   scanf("%d", &inventory[len].on_hand);
   return 1;
}
...
void search(struct part inventory[], int len)
 {
    ...
    printf("Enter part number: ");
    scanf("%d", &number);
    i = find_part(inventory, len, number);
    if (i >= 0) {
      printf("Part name %s\n", inventory[i].name);
